...
Design.phases      = {'Baseline', 'Learning'};
...
Design.abr = zeros(size(conditions));      
...
Design.nofeedback_learning = [10, 22, 30, 42, 51, 57, 66, 77, 89, 99, 109, 119, 128, 137, 146];

% trials numbers for nofeedback trials during baseline
Design.nofeedback_baseline = [4, 7, 9, 10, 15, 17, 20, 24, 27, 29];
...
for rep = 1:length(conditions)
	...
	for numb = 1:numel(list{rep})
        ...
        directory2 = [directory, all_files{numb}, '/', all_files{numb}, '_Scored.mat'];
        ...
        meanval2             = depvar(Design.learning_end-Design.learning_last+1:Design.learning_end);
        out                  = outlier(Design.learning_end-Design.learning_last+1:Design.learning_end);
        learning_mean(numb)  = mean(meanval2(out==0));
        ...
        legend([a, b], {'Veridical Feedback Trials', 'Limited Feedback'}, 'Location', 'northwest', 'FontSize', 18);
...
%         legend([a, b], {'Veridical Feedback Trials', 'Limited Feedback'}, 'Location', 'north', 'FontSize', 18);
    ...
    cnt1 = 0;
    for i = [Design.nofeedback_baseline, Design.baseline+Design.nofeedback_learning]
           
           cnt1 = cnt1+1;
        
           tmp              = total_depvar(total_outlier(:, i) == 0, i);
           tmp(isnan(tmp))  = [];
           meanCond_nofeedback(cnt1)      = mean(tmp);
           semCond_nofeedback(cnt1)       = std(tmp)/sqrt(length(tmp));
           

           tmp                       = total_depvar_biascorrected(total_outlier(:, i) == 0, i);
           tmp(isnan(tmp))           = [];
           meanCond_biasCorrected_nofeedback(cnt1) = mean(tmp);
           semCond_biasCorrected_nofeedback(cnt1)  = std(tmp)/sqrt(length(tmp)); 
...
%         close all
        ...
        for i = 1:length(list{rep})
            ...
            tmpbaselineEnd = total_depvar(i, Design.baseline_end-baseline_last+1:Design.baseline_end);
            outbaselineEnd = total_outlier(i, Design.baseline_end-baseline_last+1:Design.baseline_end);
            tmpbaselineEnd(outbaselineEnd == 1) = [];
            tmpbaselineEnd(isnan(tmpbaselineEnd)) = [];

            if ~isempty(tmpbaselineEnd)
                meantmpbaselineEnd(i, rep) = mean(tmpbaselineEnd);
            else
                meantmpbaselineEnd(i, rep) = NaN;
            end
            ...
            trlast_all   = total_depvar_biascorrected(i, Design.learning_end-Design.learning_last+1:Design.learning_end);
            outtrlast_all = total_outlier(i, Design.learning_end-Design.learning_last+1:Design.learning_end);
            trlast_all(outtrlast_all == 1) = [];
            trlast_all(isnan(trlast_all)) = [];

            if ~isempty(trlast_all)
                meantrlast_all(i, rep) = mean(trlast_all);
            else
                meantrlast_all(i, rep) = NaN;
            end

            trlast0   = total_depvar(i, Design.learning_end-Design.learning_last+1:Design.learning_end);
            outtrlast0 = total_outlier(i, Design.learning_end-Design.learning_last+1:Design.learning_end);
            trlast0(outtrlast0 == 1) = [];
            trlast0(isnan(trlast0)) = [];

            if ~isempty(trlast0)
                meantrlast0(i, rep) = mean(trlast0);
            else
                meantrlast0(i, rep) = NaN;
            end
            
            
            
            trnofeed_baseline   = total_depvar(i, Design.baseline_end+Design.nofeedback_baseline(end-baseline_lastnofeed+1:end));
            outnofeed_baseline  = total_outlier(i, Design.baseline_end+Design.nofeedback_baseline(end-baseline_lastnofeed+1:end));
            trnofeed_baseline(outnofeed_baseline == 1) = [];
            trnofeed_baseline(isnan(trnofeed_baseline)) = [];

            if ~isempty(trnofeed_baseline)
                meantrnofeed_baseline(i, rep) = mean(trnofeed_baseline);
            else
                meantrnofeed_baseline(i, rep) = NaN;
            end
            
            
            trnofeed_all   = total_depvar_biascorrected(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end));
            outnofeed_all  = total_outlier(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end));
            trnofeed_all(outnofeed_all == 1) = [];
            trnofeed_all(isnan(trnofeed_all)) = [];

            if ~isempty(trnofeed_all)
                meantrnofeed_all(i, rep) = mean(trnofeed_all);
            else
                meantrnofeed_all(i, rep) = NaN;
            end

            trnofeed0   = total_depvar(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end));
            outnofeed0  = total_outlier(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end));
            trnofeed0(outnofeed0 == 1) = [];
            trnofeed0(isnan(trnofeed0)) = [];

            if ~isempty(trnofeed0)
                meantrnofeed0(i, rep) = mean(trnofeed0);
            else
                meantrnofeed0(i, rep) = NaN;
            end
            
            aftertrnofeed0   = total_depvar(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end)+1);
            afteroutnofeed0  = total_outlier(i, Design.baseline_end+Design.nofeedback_learning(end-learning_lastnofeed+1:end)+1);
            aftertrnofeed0(afteroutnofeed0 == 1) = [];
            aftertrnofeed0(isnan(aftertrnofeed0)) = [];

            if ~isempty(aftertrnofeed0)
                aftermeantrnofeed0(i, rep) = mean(aftertrnofeed0);
            else
                aftermeantrnofeed0(i, rep) = NaN;
            end
        ...
        if Design.recall > 0
            ...
            Mtmp           = meantmp0(:, rep);
            Mtmp_biasCorrected        = meantmp_biasCorrected(:, rep);
            Mtmprecallstart = meantmprecallstart(:, rep);
        end
        

        
        Mtrlast_biasCorrected     = meantrlast_all(:, rep);
        Mtrlast        = meantrlast0(:, rep);
        Mtrnofeed_biasCorrected   = meantrnofeed_all(:, rep);
        Mtrnofeed      = meantrnofeed0(:, rep);
        Maftertrnofeed = aftermeantrnofeed0(:, rep);
        listtt          = list{rep};
        
        Mtrnofeed_baseline = meantrnofeed_baseline(:, rep);
        MtmpbaselineEnd = meantmpbaselineEnd(:, rep);
        
        if Design.normaliz == 1
            if Design.recall > 0
                save([directory, 'savedInfo_normalized.mat'], 'Design', 'listtt', 'Mtrnofeed_baseline','MtmpbaselineEnd', 'Maftertrnofeed', 'Mtmprecallstart', 'Mtmp_biasCorrected', 'Mtmp', 'Mtrlast_biasCorrected', 'Mtrlast', 'Mtrnofeed_biasCorrected', 'Mtrnofeed', 'meanCond', 'meanCond_nofeedback', 'meanCond_biasCorrected', 'meanCond_biasCorrected_nofeedback', 'semCond', 'semCond_nofeedback', 'semCond_biasCorrected', 'M10','M20', 'M30', 'M40', 'M010', 'M020', 'M030', 'M040', 'fitexp', 'fitlin', 'BCaic', 'BCbic', 'decayexp', 'decaylin');
            else
                save([directory, 'savedInfo_normalized.mat'], 'Design', 'listtt', 'Mtrnofeed_baseline', 'MtmpbaselineEnd', 'Maftertrnofeed', 'Mtrnofeed', 'Mtrnofeed_biasCorrected', 'Mtrlast', 'Mtrlast__biasCorrected', 'meanCond', 'meanCond_nofeedback', 'meanCond_biasCorrected', 'meanCond_biasCorrected_nofeedback', 'semCond', 'semCond_biasCorrected', 'semCond_nofeedback', 'semCond_biasCorrected_nofeedback');
            end 
        else  
            if Design.recall > 0
                save([directory, 'savedInfo.mat'], 'Design', 'listtt', 'Mtrnofeed_baseline','MtmpbaselineEnd', 'Maftertrnofeed', 'Mtmprecallstart', 'Mtmp_biasCorrected', 'Mtmp', 'Mtrlast_biasCorrected', 'Mtrlast', 'Mtrnofeed_biasCorrected', 'Mtrnofeed', 'meanCond', 'meanCond_nofeedback', 'meanCond_biasCorrected', 'meanCond_biasCorrected_nofeedback', 'semCond', 'semCond_nofeedback', 'semCond_biasCorrected', 'M10','M20', 'M30', 'M40', 'M010', 'M020', 'M030', 'M040', 'fitexp', 'fitlin', 'BCaic', 'BCbic', 'decayexp', 'decaylin');
            else
                save([directory, 'savedInfo.mat'], 'Design', 'listtt', 'Mtrnofeed_baseline', 'MtmpbaselineEnd', 'Maftertrnofeed', 'Mtrnofeed', 'Mtrnofeed_biasCorrected', 'Mtrlast', 'Mtrlast_biasCorrected', 'meanCond', 'meanCond_nofeedback', 'meanCond_biasCorrected', 'meanCond_biasCorrected_nofeedback', 'semCond', 'semCond_biasCorrected', 'semCond_nofeedback', 'semCond_biasCorrected_nofeedback');
            end    
        end
